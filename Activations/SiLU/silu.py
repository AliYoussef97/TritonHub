...
@triton.jit
def _silu_kernel_fwd(X, stride_X_row,
                     Y, stride_Y_row,
...
def _silu_fwd(x):
    ...
    with torch.cuda.device(x.device.index):
        _silu_kernel_fwd[grid](x, x.stride(0),
                               out, out.stride(0),
...
@triton.jit
def _silu_kernel_bwd(X, stride_X_row,
    ...
    x_sig = tl.sigmoid(x)
    dx = dout * (x_sig * (1.0 + x * (1.0 - x_sig)))
    tl.store(DX + cols, dx, mask=cols < N)

def _silu_bwd(x, dout):
    if x.stride(-1) != 1:
        x = x.contiguous()
    if dout.stride(-1) != 1:
    ...
    BLOCK_SIZE = min(MAX_FUSED_SIZE, triton.next_power_of_2(N))
    with torch.cuda.device(x.device.index):
        _silu_kernel_bwd[grid](x, x.stride(0),
                               dout, dout.stride(0),
                               dx, dx.stride(0),
                               N, BLOCK_SIZE=BLOCK_SIZE)
...
class SiLU:
    def __init__(self):
        self.silu_fn = silu.apply
    def __call__(self, x):
        return self.silu_fn(x)