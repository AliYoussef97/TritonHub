...
@triton.jit
def _tanh_kernel_fwd(X, stride_X_row,
                     Y, stride_Y_row,
...
def _tanh_fwd(x):
    ...
    with torch.cuda.device(x.device.index):
        _tanh_kernel_fwd[grid](x, x.stride(0),
                               out, out.stride(0),
...
@triton.jit
def _tanh_kernel_bwd(X, stride_X_row,
...
def _tanh_bwd(x, dout):
    ...
    BLOCK_SIZE = min(MAX_FUSED_SIZE, triton.next_power_of_2(N))
    with torch.cuda.device(x.device.index):
        _tanh_kernel_bwd[grid](x, x.stride(0),
                               dout, dout.stride(0),
                               dx, dx.stride(0),
                               N, BLOCK_SIZE=BLOCK_SIZE)
...
class tanh(torch.autograd.Function):
    ...
    def backward(ctx, d_out):
        ...
        grad = _tanh_bwd(output, d_out)
        return grad

class Tanh:
    def __init__(self):
        self.tanh_fn = tanh.apply
    def __call__(self, x):
        return self.tanh_fn(x)