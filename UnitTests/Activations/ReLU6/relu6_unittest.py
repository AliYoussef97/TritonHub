...
from TritonHub.Activations import ReLU6
...
class ReLU6UnitTest:
    ...
    def run(self):
        ...
        # interally tensors get flattened to 2D tensors (-1, C) before ReLU6 computation)
    ...
    def forward(self, input, input_ref, atol, rtol):
        output = self.ReLU6(input)
        output_ref = self.ReLU6_torch(input_ref)
    ...
    for i in range(2):
        ...
        for D in [32, 64, 128, 256, 512, 1024, 2048]:
            for dtype in [torch.float16, torch.float32, torch.float64]:
                runner = ReLU6UnitTest(B, N, M, D, dtype, print_tb)
                runner.run()
