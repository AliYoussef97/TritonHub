...
class SoftminUnitTest:
    def __init__(self, B=4, N=512, M=512, D=256, dtype=torch.float32, print_tb=False):
        ...
        # Triton Softmin and Torch Softmin
        self.Softmin = Softmin()
        self.Softmin_torch = nn.Softmin(dim=-1)
    ...
    def run(self):
        ...
        # interally tensors get flattened to 2D tensors (-1, C) before Softmin computation)
    ...
    def forward(self, input, input_ref, atol, rtol):
        output = self.Softmin(input)
        output_ref = self.Softmin_torch(input_ref)
    ...
    for i in range(2):
        ...
        for D in [32, 64, 128, 256, 512, 1024, 2048]:
            for dtype in [torch.float16, torch.float32, torch.float64]:
                runner = SoftminUnitTest(B, N, M, D, dtype, print_tb)
                runner.run()
