...
from TritonFactory.Activations import Softplus
from tabulate import tabulate as tb

class SoftplusUnitTest:
    def __init__(self, B=4, N=512, M=512, D=256, dtype=torch.float32, print_tb=False, beta=1.0, threshold=20.):
        self.B = B
        self.N = N
        self.M = M
        self.D = D
        ...
        # Triton Softplus and Torch Softplus
        self.Softplus = Softplus(beta, threshold)
        self.Softplus_torch = nn.Softplus(beta, threshold)
    ...
    def run(self):
        ...
        # interally tensors get flattened to 2D tensors (-1, C) before Softplus computation)
    ...
    def forward(self, input, input_ref, atol, rtol):
        output = self.Softplus(input)
        output_ref = self.Softplus_torch(input_ref)
    ...
    for i in range(2):
        ...
        for D in [32, 64, 128, 256, 512, 1024, 2048]:
            for dtype in [torch.float16, torch.float32, torch.float64]:
                runner = SoftplusUnitTest(B, N, M, D, dtype, print_tb, beta, threshold)
                runner.run()
