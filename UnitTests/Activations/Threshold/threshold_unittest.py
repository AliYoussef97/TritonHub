...
class ThresholdUnitTest:
    def __init__(self, B=4, N=512, M=512, D=256, dtype=torch.float32, print_tb=False, threshold=0.5, value=0.0):
        self.B = B
        self.N = N
        self.M = M
        self.D = D
        ...
        # Triton Threshold and Torch Threshold
        self.Threshold = Threshold()
        self.Threshold_torch = nn.Threshold(threshold=threshold, value=value)
    ...
    def run(self):
        ...
        # interally tensors get flattened to 2D tensors (-1, C) before Threshold computation)
    ...
    def forward(self, input, input_ref, atol, rtol):
        output = self.Threshold(input)
        output_ref = self.Threshold_torch(input_ref)
    ...
    for i in range(2):
        ...
        for D in [32, 64, 128, 256, 512, 1024, 2048]:
            for dtype in [torch.float16, torch.float32, torch.float64]:
                runner = ThresholdUnitTest(B, N, M, D, dtype, print_tb, threshold, value)
                runner.run()
