...
from Normalization.LayerNorm.layernorm import LayerNorm
...
class LayerNormUnitTest:
    def __init__(self, B=4, N=512, M=512, D=256, dtype=torch.float32, print_tb=False, eps=1e-5, bias=True, elementwise_affine=True):
        ...
        # Triton LayerNorm and Torch LayerNorm
        self.LayerNorm = LayerNorm(dimension=D, eps=eps, bias=bias, elementwise_affine=elementwise_affine, device='cuda', dtype=dtype)
        self.LayerNorm_torch = nn.LayerNorm(D, eps=eps, bias=bias, elementwise_affine=elementwise_affine, device='cuda', dtype=dtype)
    ...
    def run(self):
        ...
        # interally tensors get flattened to 2D tensors (-1, C) before LayerNorm computation)
    ...
    def forward(self, input, input_ref, atol, rtol):
        ...
        output_ref = self.LayerNorm_torch(input_ref)
    ...
    for i in range(2):
        ...
        for D in [32, 64, 128, 256, 512, 1024, 2048]:
            for dtype in [torch.float16, torch.float32, torch.float64]:
                runner = LayerNormUnitTest(B, N, M, D, dtype, print_tb, eps, bias, elementwise_affine)
                runner.run()
